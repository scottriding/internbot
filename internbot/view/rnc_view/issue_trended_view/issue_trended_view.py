...
class IssueTrendedView(ttk.Frame):
	def __init__(self, parent, controller):
		...
		self.round_dropdown.grid(row=2, column=1)
		...
		if self.check_input():
			if self.check_round():
				if self.check_output():
				 	...
				 	self.controller.run_issue_trended(input_path, input_round, output_path)
