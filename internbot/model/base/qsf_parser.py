...
class QSFQuestionsParser(object):
	...
	def __init__(self):
		...
		self.slider_parser = QSFSliderParser()
		self.__questions = []
	...
	def parse_type(self, question, question_payload, question_element):
		if question.type == 'Matrix':
			...
			slider_question = self.slider_parser.parse(question, question_payload)
...
class QSFSliderParser(object):

	def __init__(self):
		self.carryforward = QSFCarryForwardParser()

	def parse(self, question, question_payload):
		slider = self.slider_details(question_payload, question)
		if question.has_carry_forward_responses is False:
			self.basic_slider(slider, question_payload)	 
		elif question.has_carry_forward_responses is True and \
			 len(question_payload['Choices']) > 0:
			self.mixed_slider(slider, question_payload)
		else:
			self.dynamic_slider(slider, question_payload)  
		return slider

	def dynamic_slider(self, slider_question, question_payload):
		self.carryforward.assign_carry_forward(slider_question, question_payload)

	def mixed_slider(self, slider_question, question_payload):
		self.dynamic_slider(slider_question, question_payload)
		self.basic_rank(slider_question, question_payload)

	def basic_slider(self, slider, question_payload):
		if question_payload.get('Choices') and len(question_payload['Choices']) > 0:
			if question_payload.get('ChoiceOrder') and \
			   len(question_payload['ChoiceOrder']) > 0: 
				slider.question_order = question_payload['ChoiceOrder']
			self.question_details(question_payload, slider)

	def slider_details(self, question_payload, current_question):
		slider = question.CompositeSlider()
		slider.name = current_question.name
		slider.prompt = current_question.prompt
		slider.subtype = current_question.subtype
		slider.id = question_payload['QuestionID']
		return slider

	def question_details(self, question_payload, slider):
		for code, parent_question in question_payload['Choices'].items():
			sub_question = question.Question()
			sub_question.id = '%s_%s' % (slider.id, code)
			sub_question.code = code
			sub_question.type = question_payload['QuestionType']
			sub_question.subtype = question_payload['Selector']
			sub_question.name = '%s_%s' % (slider.name, code)
			sub_question.prompt = self.convert_prompt_from_byte_str(parent_question['Display'].encode('ascii', 'ignore'))
			sub_question.add_response(sub_question.prompt, code)
			slider.add_question(sub_question)
...
class QSFRankParser(object):
	...
	def mixed_rank(self, rank_question, question_payload):
		self.dynamic_rank(rank_question, question_payload)
...
class QSFCarryForwardParser(object):
	...
	def matrix_into_multiselect(self, multiselect, matrix):
		...
		for current_q in matrix.questions:
			sub_question = question.Question()
			sub_question.name = '%s_x%s' % (multiselect.name, current_q.code)
	...
	def matrix_into_rank(self, rank, matrix):
		...
		for current_q in matrix.questions:
			sub_question = question.Question()
			sub_question.name = '%s_x%s' % (rank.name, current_q.code)
	...
	def multiselect_into_rank(self, rank, matching_multi):
		...
		for sub_question in matching_multi.questions:
			current_q = question.Question()
			current_q.name = '%s_x%s' % (rank.name, sub_question.code)
	...
	def singleMulti_into_rank(self, rank, matching_MC):
		...
		for response in matching_MC.responses:
			sub_question = question.Question()
			sub_question.name = '%s_x%s' % (rank.name, response.value)
