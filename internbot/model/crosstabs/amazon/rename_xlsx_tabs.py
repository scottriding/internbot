...
class RenameTabs(object):
    ...
    def rename(self, path_to_xlsx, path_to_toc, resources_filepath):
        ...
        try:
            ...
            self.fill_cells(toc_sheet)
            self.create_toc(path_to_toc, toc_sheet)
        except:
            toc_sheet = output_workbook["TOC"]
            self.fill_cells(toc_sheet)
            self.write_table_of_contents(path_to_toc, toc_sheet, resources_filepath)
    ...
    def fill_cells(self, toc_sheet):
        for col in self.extend_alphabet:
            current_row = 1
            while current_row <= 200:
                cell = "%s%s" % (col, str(current_row))
                toc_sheet[cell].fill = self.__white_highlight
                current_row += 1

    def write_table_of_contents(self, path_to_tables, toc_sheet, resources_filepath):
        print("Writing TOC")
        logo = Image(os.path.join(resources_filepath, "amazon_xtabs.png"))
        toc_sheet.add_image(logo, "B2")
        table_hyperlink = 'B'
        table_index = 'C'
        question_name = 'D'
        ...
        with open(path_to_tables, 'r') as csvfile:
            ...
            for table_info in file:
                ...
                sheet_name = table_info['VariableName']
        ...
        name_location = str(iteration + 1)

        toc_sheet["B" + name_location].value = "=MID(CELL(\"filename\",A1),FIND(\"[\",CELL(\"filename\",A1))+1,FIND(\"]\", CELL(\"filename\",A1))-FIND(\"[\",CELL(\"filename\",A1))-1)"
        toc_sheet["B" + name_location].font = self.__font_reg

        iteration = 10
        table_index_cell = "C" + str(iteration)
        while toc_sheet[table_index_cell].value is not None:
            link = "=HYPERLINK(CONCATENATE(\"[\",$B$%s,\"]\",D%s,\"!A1\"), C%s)" % (str(name_location), str(iteration), str(iteration))
            toc_sheet[table_hyperlink + str(iteration)].value = link
            toc_sheet[table_hyperlink + str(iteration)].font = self.__font_hyper
            toc_sheet[table_hyperlink + str(iteration)].alignment = self.__center_align
            toc_sheet[table_hyperlink + str(iteration)].border = self.__thick_sides
            iteration += 1
            table_index_cell = "C" + str(iteration)
