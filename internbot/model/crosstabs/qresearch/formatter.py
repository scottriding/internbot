...
class Formatter(object):
	...
	def format_qresearch_report(self, path_to_workbook, image_path, path_to_output, is_moe = False):
		table_index = 0
		...
		# check for TOC
		try:
			self.__workbook.get_sheet_by_name("TOC")
		except KeyError:
			raise ValueError(f'Missing TOC')
		...
		for sheet in self.__workbook.worksheets:
			if sheet.title == 'TOC':
				try:
					...
					raise ValueError(f'Issue parsing TOC')
			else:
				...
				try:
					...
					raise ValueError(f'Issue formatting sheet: {title}')
				table_index += 1
		self.save(path_to_output)
	...
	def parse_toc(self, toc_sheet):
		...
		if len(self.__tables) != table_count:
			raise ValueError(f'Listed tables in TOC does not match number of tabs in .xlsx file')
